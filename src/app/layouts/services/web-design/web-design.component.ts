    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          // Quitar la clase animate cuando el círculo no es visible
          entry.target.classList.remove('animate');
        }
      });
    }, {
      threshold: 0.1, // Animar cuando al menos el 10% del elemento es visible (más sensible)
      // rootMargin: '0px 0px -100px 0px' // Eliminado para evitar que el margen de la raíz interfiera
    });
